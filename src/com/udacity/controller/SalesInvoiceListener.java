...
public class SalesInvoiceListener implements ActionListener, ListSelectionListener  {
    ...
    public void valueChanged(ListSelectionEvent e) {
        ...
        invoicesTableRwSelect();
    }

    private void invoicesTableRwSelect() {
        ...
        if (selectedRowIndex >= 0) {
            ...
            frame.getInvDateTF().setText(df.format(row.getInvDte()));
    ...
    private void createLineOK() {
        String itmName = frame.getLineDialog().getItemNameField().getText();
        String itmCountStr = frame.getLineDialog().getItemCountField().getText();
        String itmPriceStr = frame.getLineDialog().getItemPriceField().getText();
        frame.getLineDialog().setVisible(false);
        frame.getLineDialog().dispose();
        frame.setLineDialog(null);
        int itemCount = Integer.parseInt(itmCountStr);
        ...
        InvoiceLine invoiceLine = new InvoiceLine(itmName, itemPrice, itemCount, invoice);
    ...
    private void deleteLine() {
        int lineIndx = frame.getInvLinesTable().getSelectedRow();
        InvoiceLine line = frame.getInvoiceLinesTableModel().getInvoiceLines().get(lineIndx);
        frame.getInvoiceLinesTableModel().getInvoiceLines().remove(lineIndx);
        frame.getInvoiceLinesTableModel().fireTableDataChanged();
        frame.getInvoiceHeaderTableModel().fireTableDataChanged();
